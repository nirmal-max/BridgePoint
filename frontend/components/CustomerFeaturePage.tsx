...
export default function CustomerFeaturePage({ section }: { section: Section }) {
  ...
  useEffect(() => { if (!authLoading && !user) router.replace(`/signup?role=customer&next=${encodeURIComponent(`/${section}`)}`); }, [authLoading, router, section, user]);
