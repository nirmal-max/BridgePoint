...
export default function CustomerFeaturePage({ section }: { section: Section }) {
  ...
  useEffect(() => { if (!authLoading && !user) router.replace(`/signup?role=customer&next=${encodeURIComponent(`/${section}`)}`); else if (!authLoading && user && (user.is_admin || user.roles?.includes("cooperative") || user.roles?.includes("labor") || user.role === "labor" || user.labor_category)) router.replace(user.is_admin || user.roles?.includes("cooperative") ? "/admin" : "/worker"); }, [authLoading, router, section, user]);
