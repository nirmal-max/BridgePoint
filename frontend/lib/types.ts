...
export interface TrustScore { worker_id: number; trust_score: number; completed_jobs: number; assigned_jobs: number; average_rating?: number | null; verified_certifications: number; completion_rate: number; cancellation_rate: number; method: string; }
...
export interface Invoice { id: number; job_id: number; invoice_number: string; employer_id: number; worker_id?: number | null; employer_name: string; worker_name?: string | null; service: string; job_date: string; amount: number; commission: number; total: number; payment_status: string; transaction_reference?: string | null; created_at: string; }
